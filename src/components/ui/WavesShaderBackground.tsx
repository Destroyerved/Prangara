...
export const WavesShaderBackground: React.FC<WavesShaderProps> = ({
  className = "",
}) => {
  ...
  useEffect(() => {
    ...
    // Fullscreen single triangle: covers [-1, -1] to [3, -1], [-1, 3]
    ...
    // Colours (low -> high):
    // #031C26 -> vec3(0.012, 0.110, 0.149)
    // #1B6CA8 -> vec3(0.106, 0.424, 0.659)
    // #5AD2F4 -> vec3(0.353, 0.824, 0.957)
    // #EAF9FF -> vec3(0.918, 0.976, 1.000)
    ...
    const resize = () => {
      if (!canvas) return;
      // Cap devicePixelRatio at 2 as instructed
    ...
    const render = (now: number) => {
      ...
      // Uniforms as specified in header comment:
      // u_colors[8]
      ...
      // u_shape = vec4(2.00, 0.54, 0.47, 0.04) (scale, intensity, paramA, warp)
      if (uShapeLoc) gl.uniform4f(uShapeLoc, 2.00, 0.54, 0.47, 0.04);

      // u_surface = vec4(1.54, 1.16, 0.00, 1.00) (detail, contrast, brightness, saturation)
      if (uSurfaceLoc) gl.uniform4f(uSurfaceLoc, 1.54, 1.16, 0.00, 1.00);

      // u_finish = vec4(0.00, 0.21, 0.002, 0.10) (hue, vignette, blur, grain)
      if (uFinishLoc) gl.uniform4f(uFinishLoc, 0.00, 0.21, 0.002, 0.10);

      // u_transform = vec4(4012.0, 5.65, 0.12, 0.0) (seed, rotation, drift, OKLab)
      if (uTransformLoc) gl.uniform4f(uTransformLoc, 4012.0, 5.65, 0.12, 0.0);

      // u_space = vec4(0.11, -0.19, pointer x, pointer y)
      if (uSpaceLoc) gl.uniform4f(uSpaceLoc, 0.11, -0.19, 0.0, 0.0);
  ...
  return (
    <div
      className={`waves-shader-container ${className}`}
      aria-hidden="true"
    >
      <canvas ref={canvasRef} className="waves-shader-canvas" />
