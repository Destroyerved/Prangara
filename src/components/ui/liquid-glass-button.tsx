...
const liquidbuttonVariants = cva(
  "inline-flex items-center justify-center cursor-pointer gap-2 whitespace-nowrap rounded-full text-sm font-medium transition-[color,box-shadow,transform,filter] disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 shrink-0 [&_svg]:shrink-0 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive",
  {
    variants: {
      variant: {
        default: "bg-transparent hover:scale-[1.02] duration-300 transition text-primary",
        blue: "flow-button-blue border-[#38bdf8]/70 bg-gradient-to-r from-[#0284c7] via-[#0ea5e9] to-[#38bdf8] !text-white text-white shadow-[0_4px_18px_rgba(2,132,199,0.48)] hover:shadow-[0_6px_28px_rgba(56,189,248,0.68)] hover:border-[#38bdf8] transition-all duration-300",
        destructive:
...
function LiquidButton({
  ...
  const Comp: React.ElementType = asChild ? Slot : to ? Link : "button"
  ...
  const compProps = to ? { to, style: mergedStyle, ...props } : { style: mergedStyle, ...props }

  const label = text || children
  const shouldShowArrow = showArrow ?? (variant === "blue" && Boolean(text))
  ...
  return (
    <>
      <Comp
        data-slot="button"
        className={cn(
          "relative overflow-hidden group select-none leading-none",
          liquidbuttonVariants({ variant, size, className })
        )}
        {...compProps}
      >
        {/* Softened liquid glass bead reflection - subtle, non-blinding shine */}
        <div
          className={cn(
            "absolute inset-0 z-[1] h-full w-full rounded-full transition-all pointer-events-none",
            subtleShine
              ? "shadow-[inset_2px_2px_1px_-1px_rgba(255,255,255,0.45),inset_-2px_-2px_1px_-1px_rgba(255,255,255,0.3),inset_0_0_6px_rgba(255,255,255,0.18)]"
              : "shadow-[inset_3px_3px_2px_-1px_rgba(255,255,255,0.85),inset_-3px_-3px_2px_-1px_rgba(255,255,255,0.85),0_0_12px_rgba(255,255,255,0.3)]"
          )}
        />
        <div
          className="absolute inset-0 z-0 h-full w-full overflow-hidden rounded-full pointer-events-none opacity-40"
        ...
        {shouldShowArrow && (
          <ArrowRight
            className="relative z-[3] shrink-0 w-4 h-4 stroke-white !stroke-white transition-transform duration-300 ease-out group-hover:translate-x-1"
            strokeWidth={2.2}
          />
        )}
        <GlassFilter />
