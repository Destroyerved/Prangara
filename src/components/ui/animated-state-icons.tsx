...
export function ToggleIcon({ size = 40, color = "currentColor", className, duration = 1800, active }: StateIconProps) {
  const autoOn = useAutoToggle(duration);
  const on = active !== undefined ? active : autoOn;
  return (
    <svg viewBox="0 0 40 40" fill="none" className={cn("", className)} style={{ width: size, height: size }}>
...
/* ─── 13. OVERVIEW ─── 4-grid squares morphing */
export function OverviewIcon({ size = 40, color = "currentColor", className, duration = 2400 }: StateIconProps) {
  const active = useAutoToggle(duration);
  return (
    <svg viewBox="0 0 40 40" fill="none" className={cn("", className)} style={{ width: size, height: size }}>
      <motion.rect x="8" y="8" width="10" height="10" rx="2.5" stroke={color} strokeWidth={2}
        animate={active ? { scale: [1, 1.12, 1], fill: color, fillOpacity: 0.2 } : { scale: 1, fill: "none", fillOpacity: 0 }}
        transition={{ duration: 0.4 }}
        style={{ transformOrigin: "13px 13px" }}
      />
      <motion.rect x="22" y="8" width="10" height="10" rx="2.5" stroke={color} strokeWidth={2}
        animate={active ? { y: [8, 6, 8] } : { y: 8 }}
        transition={{ duration: 0.4, delay: 0.08 }}
      />
      <motion.rect x="8" y="22" width="10" height="10" rx="2.5" stroke={color} strokeWidth={2}
        animate={active ? { x: [8, 6, 8] } : { x: 8 }}
        transition={{ duration: 0.4, delay: 0.16 }}
      />
      <motion.rect x="22" y="22" width="10" height="10" rx="2.5" stroke={color} strokeWidth={2}
        animate={active ? { scale: [1, 1.12, 1], fill: color, fillOpacity: 0.2 } : { scale: 1, fill: "none", fillOpacity: 0 }}
...
/* ─── 14. FOOTPRINT ─── 3 stacked geometric layers */
export function FootprintIcon({ size = 40, color = "currentColor", className, duration = 2400 }: StateIconProps) {
  const shifted = useAutoToggle(duration);
  return (
    <svg viewBox="0 0 40 40" fill="none" className={cn("", className)} style={{ width: size, height: size }}>
      <motion.path d="M8 14l12-6 12 6-12 6-12-6z" stroke={color} strokeWidth={2} strokeLinejoin="round"
        animate={shifted ? { y: -2 } : { y: 0 }}
        transition={{ duration: 0.4 }}
      />
      <motion.path d="M8 21l12 6 12-6" stroke={color} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"
        animate={shifted ? { y: 1 } : { y: 0 }}
        transition={{ duration: 0.4, delay: 0.08 }}
      />
      <motion.path d="M8 28l12 6 12-6" stroke={color} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"
        animate={shifted ? { y: 3 } : { y: 0 }}
        transition={{ duration: 0.4, delay: 0.16 }}
      />
    </svg>
  );
...
/* ─── 15. ABATEMENT PORTFOLIO ─── animated chart bars & trend line */
export function AbatementPortfolioIcon({ size = 40, color = "currentColor", className, duration = 2200 }: StateIconProps) {
  ...
  return (
    <svg viewBox="0 0 40 40" fill="none" className={cn("", className)} style={{ width: size, height: size }}>
      ...
      <motion.path d="M8 22l8-7 8 4 8-10" stroke={color} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"
        animate={high ? { opacity: 1, pathLength: 1 } : { opacity: 0.4, pathLength: 0.7 }}
        transition={{ duration: 0.4 }}
      />
    </svg>
  );
