...
export function adaptAssessment(raw:unknown,input?:unknown,id?:string) {
  ...
  return {id:id||'engine-'+str(object(r.profile).sector),plant:adaptPlant(input,undefined,assumptions),origin:{kind:'engine',note:r.is_demo?'Engine demonstration; not a measured factory assessment.':'Calculated by the configured PRANGARA engine.',reference:id||'Unpersisted sandbox assessment'},
    ...
    compliance:{cbam:{applicability:cbam.applicable?'Potentially applicable — screening only':'Not indicated by sector screening',exposure_t:optional(cbam.embedded_emissions_exported_tco2e),indicative_cost:optional(cbam.indicative_annual_cost_inr),reference_price:optional(cbam.reference_price_inr_per_tco2e),export_share_pct:optional(cbam.eu_export_share_pct),included:['Scope 1','Scope 2'],excluded:['Precursor and upstream material emissions'],assumptions:[cbam.basis,cbam.caveat]},brsr:list(brsr.readiness).map(b=>({name:b.item,status:b.status==='ready'?'Ready':b.status==='partial'?'Partial':b.status==='no data'?'Missing':'External action required',detail:b.note||brsr.why}))},
